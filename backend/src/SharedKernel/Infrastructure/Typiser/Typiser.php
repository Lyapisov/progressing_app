...
final class Typiser implements TypiserInterface
{
    ...
    public function toInt($value): int
    {
        ...
        if (is_float($value))   return intval(round($value));
        if (is_numeric($value)) return intval(round($value));
    ...
    public function toFloat($value, $precision = 2): float
    {
        ...
        if (is_int($value))     return round($value, $precision);
        if (is_null($value))    return round(0, $precision);
        if (!is_scalar($value)) return round(0, $precision);
        if (is_numeric($value)) return round($value, $precision);
        if (is_string($value))  return round(0, $precision);
